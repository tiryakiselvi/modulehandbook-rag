...
python -m modulehandbook_rag.cli benchmark \
  --retrievers bm25 \
  --top-k 3 \
  --section-boosts 8 \
  --out-csv outputs/evaluation_results.csv \
  --out-md docs/evaluation_results.md
...
python -m modulehandbook_rag.cli benchmark \
  ...
  --require-section \
  --section-boosts 0,8 \
  --out-csv outputs/evaluation_results_strict.csv \
  --out-md docs/evaluation_results_strict.md
...
python -m modulehandbook_rag.cli benchmark \
  ...
  --by-query-type \
  --section-boosts 8 \
  --out-csv outputs/evaluation_by_type.csv \
  --out-md docs/evaluation_by_type.md
...
python -m modulehandbook_rag.cli benchmark \
  ...
  --top-k 3 \
  --require-section \
  --out-csv outputs/evaluation_multilingual.csv \
  --out-md docs/evaluation_multilingual.md
