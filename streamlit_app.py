...
with st.sidebar:
    ...
    retriever_name = st.selectbox("Retrieval", ["BM25", "Hybrid", "Dense"], index=0, key="retriever_name_v2")
    top_k = st.slider("Trefferanzahl", min_value=1, max_value=8, value=3)
    alpha = st.slider("Hybrid-Gewichtung", 0.0, 1.0, 0.5, 0.05)
    section_boost = st.slider("Abschnittsgewichtung", 0.0, 10.0, 8.0, 0.5, key="section_boost_v2")
